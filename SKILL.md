...
- Reference to a named IP (character, franchise, game title, studio) — text or attached image → the agent runs the **6-phase IP-reference process** (Step 1 below) to translate the named reference into original, abstract visual traits before calling `image_gen`. Hardware / era / genre descriptors like "NES-style", "SNES-era", "Game Boy-style", "GBA-era", "arcade beat'em-up", and "16-bit JRPG" are allowed style tokens when they improve fidelity. The final prompt contains NO protected identity / source-IP names, and the output is a genuinely original asset inspired by the user's intent.

The 6-phase process is universal — it applies to ANY protected identity / source-IP reference the user might mention, without needing a curated list of "famous characters." Concrete style descriptors, not protected IP names, are what reach the image generator.
...
When the user gives a style hint (era / mood / genre), the agent draws descriptors from `references/visual-vocabulary.md`. **There is no curated anchor library and no list of named games** — the agent uses world knowledge to interpret the user's intent, keeps useful hardware / era / genre tokens, translates studios and specific game titles into visual traits, and writes the prompt with concrete style dimensions (era, hardware constraints, palette family, silhouette, mood). Variety comes from the user's prompt + the agent's enrichment + the descriptor set, not from named-game presets.
...
This is where the agent earns its keep. Read the user's natural-language input and translate it into a rich `Subject:` line for the imagegen prompt — using concrete video-game art vocabulary while keeping protected identity / source-IP names out of the final prompt.

**The core rule: no protected identity / source-IP names reach `image_gen`.** Character names, franchise names, studio / publisher names, and specific game titles are translated during enrichment. Hardware / era / genre descriptors are allowed when they improve style fidelity: `NES-style`, `SNES-era`, `Game Boy-style`, `GBA-era`, `arcade`, `beat'em-up`, `16-bit JRPG`, `metroidvania`, etc. The agent uses world knowledge in Phases 1-4 to interpret what the user meant; Phase 5 writes the prompt using allowed style tokens plus concrete descriptors drawn from `references/visual-vocabulary.md`. Phase 6 preflight scans for leaked protected references and semantic lookalike clusters before the prompt is sent.

This is a universal process — it does not depend on a curated list of famous characters. Any named character / franchise / studio / specific game the user might mention is handled by the same six phases. Hardware / era / genre words are classified separately and may be kept.

#### Phase 1 — Detect style and IP references

Scan the user's input for any protected/source-IP reference and for any useful style token. Treat the following as **reference-only**, not final prompt text:

- **Character names**: named mascot characters, named superheroes, named fantasy heroes, named anime / game characters.
- **Franchise / IP names**: named game, comics, film, anime, or toy franchises.
- **Studio / publisher names**: named game studios, publishers, comics publishers, animation studios.
- **Game titles**: specific named games, sequels, expansions, or series entries.
- **Visible-in-reference-image IP**: if a user attaches an image, mentally label any recognizable IP in it.

Also identify allowed style tokens that may remain in the final prompt:

- **Hardware / era names**: NES-style, SNES-era, Game Boy-style, GBA-era, Genesis-era, DOS VGA, PC-98, Pico-8, 8-bit, 16-bit, 32-bit.
- **Genre / format names**: arcade, beat'em-up, JRPG, platformer, fighting game, metroidvania, creature-collection RPG, roguelike, tactics RPG, top-down RPG, isometric dungeon-crawler.

If there are no protected references and no reference image → skip to normal enrichment (use `visual-vocabulary.md` for directional hints like "16-bit JRPG", "arcade beat'em-up", or "cyberpunk").
...
|---|---|---|---|
| **exact** | "draw [protected character]", "make [protected mascot]", "the actual [named game hero]" | HIGH | Phase 4 mandatory: make a clearly original character with shifted identity traits. |
...
| **style-anchor** | User names a game/franchise as a STYLE anchor ("in the style of [specific game title]") — and the request is NOT for a character from that game | MEDIUM | Phase 4: translate the game-name into abstract era / hardware / palette / mood descriptors. |
| **studio-anchor** | User names a studio / publisher as a STYLE anchor ("[studio] style", "[publisher] arcade") | MEDIUM | Phase 4: translate studio name into era / genre / palette / anatomy / outline descriptors. |
| **era-only** | User names hardware ("NES style", "Game Boy palette") or a generic era descriptor | LOW | Keep the hardware / era token if useful, and reinforce it with concrete descriptors (e.g., "NES-style 8-bit console sprite, 3-4 colors per sprite, chibi 1:2 proportions"). |
...
4. **Distinctive motifs** — what makes the subject visually identifiable WITHOUT using the protected name. Avoid one-to-one replacement descriptions that preserve a famous character's full identity cluster; transform the motif set into a new silhouette, color family, markings, and power presentation.
5. **Emotional tone** — see `visual-vocabulary.md` → "Mood / tone vocabulary" (cute / heroic / grim / cyberpunk / playful / mystical / gritty / regal).
6. **Era / hardware / genre** — see `visual-vocabulary.md` → "Era / hardware vocabulary" (NES-style 8-bit console / Game Boy DMG / SNES-era 16-bit console / arcade beat'em-up / modern indie chunky / HD-2D / etc.). Hardware / era / genre descriptors are allowed in the final prompt; studio names and specific game titles are translated.
7. **Gameplay role** — see `visual-vocabulary.md` → "Genre / role vocabulary" (JRPG protagonist / beat'em-up brawler / creature-collection companion / cyberpunk hacker / dark-fantasy hunter / cozy farm villager / etc.).
...
4. **Named powers → generic effects** — translate franchise-specific abilities into broad elemental or technology effects, then relocate or restyle them so they do not preserve the named character's signature presentation.
...
**Semantic lookalike clusters:** Some prompts can trip guardrails even after literal names are removed because the remaining descriptors still recreate a protected subject too closely. Treat these clusters as HIGH-risk and transform harder. Ask: does the combination of asset type, species/body taxonomy, palette, signature markings, power/effect placement, costume layout, role framing, pose, and era point strongly back to one protected character, franchise, studio, or game title? If yes, transform at least 3 identity levers before calling `image_gen`.

For MEDIUM-risk style-anchor / studio-anchor: typically only need lever 6 (translate to era / palette / silhouette / outline / anatomy descriptors). For LOW-risk era-only: keep the useful hardware / era / genre token and reinforce it with concrete descriptors.
...
Compose the complete structured prompt (see `references/concept-prompt-template.md`) using the dimensions from Phase 3, the transformed traits from Phase 4, and allowed hardware / era / genre tokens. Do NOT paste the raw user request into any field when it contains protected/source-IP names. NO protected identity / source-IP names appear in:
...
Not even in negative clauses like "not [protected character]" or "no [franchise] logo" — those still trip guardrails. The model never sees the protected name; it sees the abstract description.

**Style-token policy.**

- Allowed in final prompt: hardware / era / genre descriptors such as "NES-style", "SNES-era", "Game Boy-style", "GBA-era", "Genesis-era", "arcade beat'em-up", "late-arcade 2D fighter", "16-bit JRPG", "metroidvania", "creature-collection RPG", and similar broad gameplay categories. If a genre phrase combines with other traits into a recognizable protected identity cluster, keep the broad gameplay intent but transform the identity-bearing traits.
- Translate before final prompt: studio / publisher names.
- Translate before final prompt: specific game titles.
- Transform before final prompt: named characters / franchises / logos / signature identity symbols.

Always pair allowed style tokens with concrete descriptors. Write "NES-style 8-bit console sprite, 3-4 colors per sprite, bold 1-pixel outlines" rather than only "NES style"; write "SNES-era 16-bit JRPG sprite, ~16-24 colors, chibi 1:3 proportions with dithered shading" rather than only "SNES style".

**Allowed broad genre / archetype language** (not protected names — fine to use): "armored techno-brawler", "elemental fantasy critter", "colorful platform mascot hero", "creature-collection companion", "side-scrolling beat'em-up character". These are safe only when they are not paired with a full protected identity cluster.

#### Phase 6 — Preflight scan

Before the prompt goes to `image_gen`, run a 5-check scan on the **entire structured prompt**, not only the `Subject:` line:

1. **Leaked protected/source-IP names** — does the prompt contain any character, franchise, studio / publisher, specific game title, logo, or named identity symbol from Phase 1's detected list? If yes, return to Phase 5. Allowed hardware / era / genre tokens are fine.
...
5. **Semantic lookalike cluster** — even with no names, does the prompt combine enough traits to strongly imply a protected character, franchise, studio style, or specific game title? If yes, transform more before sending.

If any check fails → return to the appropriate phase and re-run before sending. Do not retry `image_gen` with the same unsafe prompt.
...
- Don't put protected character / franchise / studio / publisher / specific game names into the final `image_gen` prompt, even inside "not copying X" or "no X logo" clauses. Hardware / era / genre descriptors are allowed style tokens.
...
**Save** the agent's enriched prompt to `tmp/prompt.md`. This file is the exact safe prompt sent to `image_gen`; it must not include raw protected/source-IP user text. This is for the user's reference — useful for debugging (compare against what `image_gen` produced), iterating (tweak by hand and re-run), or auditing the agent's translation work (especially for HIGH-risk IP-reference cases where the agent ran the full 6-phase process).
...
| **32** | arcade fighters, modern indie painterly, atmospheric color-rich subjects | arcade 16-bit fighter, late-arcade 2D fighter, modern indie painterly, cyberpunk pixel-art, dark fantasy / soulslike, dark fantasy isometric dungeon-crawl |
...
- Don't put protected identity / source-IP names in the `image_gen` prompt — not character names, not franchise names, not studio / publisher names, not specific game titles. The 6-phase process (Step 1) translates those into descriptive style traits before the prompt is sent. Hardware / era / genre descriptors are allowed when useful. Even "not [protected character]" or "no [franchise] logo" negative clauses still trip the IP guardrail — the model never sees the protected name.
- Don't skip Phase 6 preflight. It's the safety net that catches accidentally leaked protected/source-IP names before the prompt reaches `image_gen`.

## When something goes wrong

See `references/failure-modes.md`. Common cases:

- Built-in image generation disconnects before completion → commonly a **preflight failure** (Phase 6 missed a protected/source-IP name), but backend stream failures can also happen for unrelated reasons. Check `$CODEX_HOME/generated_images/...` for a newly written image; if a file exists, show it for Step 3 review. If no file exists, return to Phase 6 of Step 1, scrub the prompt for leaked protected names (character names, franchise names, studio / publisher names, specific game titles), and retry with a clean descriptive prompt. If it still fails, explain that the built-in image tool is failing and ask before using the imagegen CLI fallback, which requires `OPENAI_API_KEY`.
